 ...
 */
boilerUtils.getBoilerTemplateData = function getTemplateData (includes, action, config, appScript) {
  return {
    /* The assets/loader script isn't hooked using the public object, to asure, it's not removed. */
    script: boilerUtils
      .stringifyFuncCaller([boilerUtils.getLoader()]
        .concat(config.js || ''))
      .join('\n'),
    css: ['"', '"']
      .join(
    ...
    appScript: appScript,
    onLoadHooks: ['[', ']']
      .join(boilerUtils.parseJsArr([]
        .concat(config.onLoad || ''))
        .join(',')),
