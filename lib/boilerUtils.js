...
/* boilerUtils.runBeforeRenderHooks(hooks)
 * `hooks` - An array of functions
 */
boilerUtils.runBeforeRenderHooks = function runBeforeRenderHooks (hooks, config) {
  if(!(func = hooks.shift())) return config
  if(typeof func === 'object') boilerUtils.runBeforeRenderHooks(hooks, _.extend(func, config))
  return boilerUtils.runBeforeRenderHooks(hooks, func(config))
}
