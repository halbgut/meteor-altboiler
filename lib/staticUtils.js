...
StaticUtils.server = function StaticUtilsServer (route) {
  ...
  return function StaticUtilsServe (req, res) {
    ...
    if(file.cachable) {
      ...
      if(self.shouldUseCache(req.headers, file.etag)) {
        res.writeHeader(304)
        res.end('')
      } else {
        res.write(file.file)
...
StaticUtils.shouldUseCache = function StaticUtilsShouldUseCache (reqHeaders, etag) {
  console.log(reqHeaders['cache-control'])
  return (
